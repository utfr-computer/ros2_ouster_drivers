...
void OusterDriver::processData() {
  ...
  while (_processing_active) {
    ...
    _process_cond.wait(ringbuffer_guard, [this]() {
      return (!_lidar_packet_buf->empty() || !_imu_packet_buf->empty() ||
              !_processing_active);
    });
    ...
    if (!_lidar_packet_buf->empty() && _processing_active) {
      _full_rotation_accumulator->accumulate(_lidar_packet_buf->head(),
                                             override_ts);
      for (DataProcessorMapIt it = key_lidar_its.first;
           it != key_lidar_its.second; it++) {
    ...
    if (!_imu_packet_buf->empty() && _processing_active) {
      for (DataProcessorMapIt it = key_imu_its.first; it != key_imu_its.second;
           it++) {
...
void OusterDriver::receiveData() {
  while (_processing_active) {
    try {
      ...
      bool got_lidar =
          _sensor->readLidarPacket(state, _lidar_packet_buf->tail());
      ...
      if (got_lidar || got_imu) {
        if (got_lidar) {
          // If the ringbuffer is full, this means the processing thread is
          // running too slow to process all frames. Therefore, we push to it
          // anyway, discarding all (old) data in the buffer and emit a warning.
      ...
      RCLCPP_DEBUG(this->get_logger(), "Retrieved packet with state: %s",
...
void OusterDriver::getMetadata(
  ...
  if (request->metadata_filepath != "") {
    ...
    ofs.close();
    if (!ofs) {
      RCLCPP_ERROR(this->get_logger(), "Failed to save metadata to: %s.",
