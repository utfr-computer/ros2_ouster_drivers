...
void SensorTins::reset(ros2_ouster::Configuration &config,
                       rclcpp_lifecycle::LifecycleNode::SharedPtr node) {
  _ouster_client.reset();
  configure(config, node);
}

void SensorTins::configure(ros2_ouster::Configuration &config,
                           rclcpp_lifecycle::LifecycleNode::SharedPtr node) {
  RCLCPP_INFO(node->get_logger(), "Configuring Tins-based Ouster driver node.");

  // Declare parameters specific to the SensorTins implementation
  ros2_ouster::declare_parameter_if_not_declared(
      node, "ethernet_device",
      rclcpp::ParameterValue("no_ethernet_device_configured"));
  ros2_ouster::declare_parameter_if_not_declared(
      node, "metadata_filepath",
      rclcpp::ParameterValue("no_filepath_specified"));
  ...
  try {
    ...
    config.metadata_filepath =
        node->get_parameter("metadata_filepath").as_string();
  } catch (...) {
    throw ros2_ouster::OusterDriverException(
        "TinsDriver failed to retrieve the ethernet_device or "
        "metadata_filepath parameter");
    exit(-1);
  }
  ...
  RCLCPP_INFO(
      ...
      config.lidar_ip.c_str(), config.computer_ip.c_str());
  ...
  loadSensorInfoFromJsonFile(_driver_config.metadata_filepath, _metadata);
  ...
  try {
    ...
    throw ros2_ouster::OusterDriverException(
        "Failed to configure Tins sniffer. Check that the entered ethernet "
        "device " +
...
ouster::sensor::client_state SensorTins::get() {
  ...
  _tins_sniffer_pointer->sniff_loop(
      std::bind(&SensorTins::sniffOnePacket, this, std::placeholders::_1));

  // Return the state, as best can be inferred from the data
  return _inferred_state;
}

bool SensorTins::readLidarPacket(const ouster::sensor::client_state &state,
                                 uint8_t *buf) {
  if (state == ouster::sensor::client_state::LIDAR_DATA) {
    std::memcpy(buf, _lidar_packet.data(),
                this->getPacketFormat().lidar_packet_size);
    return true;
  } else {
    return false;
  }
...
bool SensorTins::readImuPacket(const ouster::sensor::client_state &state,
                               uint8_t *buf) {
  if (state == ouster::sensor::client_state::IMU_DATA) {
    std::memcpy(buf, _imu_packet.data(),
                this->getPacketFormat().imu_packet_size);
    return true;
  } else {
    return false;
  }
...
ros2_ouster::Metadata SensorTins::getMetadata() { return _metadata; }
...
void SensorTins::loadSensorInfoFromJsonFile(
    std::string filepath_to_read, ouster::sensor::sensor_info &sensor_info) {
  if (filepath_to_read == "") {
    throw ros2_ouster::OusterDriverException(
        "Metadata filepath is empty! The Tins driver needs a valid metadata "
        "file!");
    exit(-1);
  }
...
void SensorTins::initializeSniffer(const std::string eth_device) {
  ...
  _tins_sniffer_pointer =
      std::make_unique<Tins::Sniffer>(eth_device, _sniffer_config);
}
