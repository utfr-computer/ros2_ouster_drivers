...
std::string to_string(OperatingMode mode) {
  ...
  auto res = std::find_if(operating_mode_strings.begin(), end,
...
sensor_config parse_config(const Json::Value &root) {
  ...
  if (!root["multipurpose_io_mode"].empty()) {
    auto multipurpose_io_mode =
        multipurpose_io_mode_of_string(root["multipurpose_io_mode"].asString());
