...
static std::string auto_start_string(OperatingMode mode) {
  ...
  auto res = std::find_if(auto_start_strings.begin(), end,
...
bool set_config_helper(SOCKET sock_fd, const sensor_config &config,
  ...
  auto set_param = [&sock_fd, &res](std::string param_name,
    ...
    bool success = true;
    success &=
        do_tcp_cmd(sock_fd, {"set_config_param", param_name, param_value}, res);
...
std::shared_ptr<client> init_client(const std::string &hostname,
  ...
  // If udp_dest_host is empty string, use automatic addressing with
  // set_udp_dest_auto
  if (udp_dest_host != "") {
  ...
  success &= do_tcp_cmd(sock_fd,
                        {"set_config_param", "operating_mode", "NORMAL"}, res);
  success &= res == "set_config_param";
