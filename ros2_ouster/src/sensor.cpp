...
Sensor::~Sensor() { _ouster_client.reset(); }

void Sensor::reset(ros2_ouster::Configuration &config,
                   rclcpp_lifecycle::LifecycleNode::SharedPtr node) {
  _ouster_client.reset();
  configure(config, node);
}

void Sensor::configure(ros2_ouster::Configuration &config,
                       rclcpp_lifecycle::LifecycleNode::SharedPtr node) {
  RCLCPP_INFO(node->get_logger(), "Configuring Ouster driver node.");
  ...
  RCLCPP_INFO(node->get_logger(), "Connecting to sensor at %s.",
              config.lidar_ip.c_str());
  if (config.computer_ip == "") {
    RCLCPP_INFO(node->get_logger(), "Sending data from sensor to computer "
                                    "using automatic address detection");
  } else {
    RCLCPP_INFO(node->get_logger(), "Sending data from sensor to %s.",
...
bool Sensor::readLidarPacket(const ouster::sensor::client_state &state,
                             uint8_t *buf) {
  if (state & ouster::sensor::client_state::LIDAR_DATA &&
      ouster::sensor::read_lidar_packet(*_ouster_client, buf,
...
bool Sensor::readImuPacket(const ouster::sensor::client_state &state,
                           uint8_t *buf) {
  if (state & ouster::sensor::client_state::IMU_DATA &&
      ouster::sensor::read_imu_packet(*_ouster_client, buf,
                                      this->getPacketFormat())) {
    return true;
  }
  return false;
